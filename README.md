...
List emoji in one Discord server, fetch selected static emoji as visual-reference image data, and prepare/upload images generated by the host agent. Returns a Discord mention for immediate use and only removes emoji that Discord reports were created by the authenticated bot. The server itself does not generate images.
