...
List emoji in one Discord server, fetch selected static emoji as visual-reference image data, and prepare/upload bounded images generated by the host agent from exactly one base64/data-URL or absolute local-path/file-URI input. Returns a Discord mention for immediate use and only removes emoji that Discord reports were created by the authenticated bot. The server itself does not generate images.

**5 tools:** `list_server_emojis`, `fetch_emoji_reference`, `upload_emoji`, `list_created_emojis`, `remove_emoji`

**Requires:** Discord bot token with `CREATE_GUILD_EXPRESSIONS`, Discord guild ID
