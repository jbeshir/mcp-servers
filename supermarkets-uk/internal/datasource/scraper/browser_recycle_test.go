...
// without launching a real Chrome process. The supermarkets-uk Browser is
// shared across SearchAll's concurrent per-store fetches, so the concurrency
// test below is the important one: it proves the force-kill is idempotent per
// generation and cannot double-kill or race when several in-flight fetches
// recycle the same poisoned browser at once.
...
// TestRecycleIsIdempotentUnderConcurrency models SearchAll's fan-out: many
// concurrent fetches on the same shared Browser all hit the recycle path for
// the same poisoned generation. The process must be killed exactly once.
